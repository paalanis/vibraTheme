...
// Types: s(nombre) i×6(marca,genero,tipo,talle,color,iva) s(foto) d×2(costo,margen) i(id) → 'siiiiiisd di'
$stmt = mysqli_prepare($conexion,
    "UPDATE tb_productos
     SET nombre=?, id_marca=?, id_genero=?, id_tipo=?, id_talle=?, id_color=?,
         id_iva_condicion=?, foto=?, precio_costo=?, margen_ganancia=?
     WHERE id_productos=?"
);
mysqli_stmt_bind_param($stmt, 'siiiiiisddi',
    $nombre, $marca, $genero, $tipo, $talle, $color,
    $iva, $foto, $costo, $margen, $id
);
