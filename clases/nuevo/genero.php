...
$stmt = mysqli_prepare($conexion, "SELECT id_genero, nombre FROM tb_genero ORDER BY nombre ASC");
mysqli_stmt_execute($stmt);
mysqli_stmt_bind_result($stmt, $g_id, $g_nombre);
$generos = [];
while (mysqli_stmt_fetch($stmt)) {
    $generos[] = ['id' => $g_id, 'nombre' => htmlspecialchars($g_nombre, ENT_QUOTES, 'UTF-8')];
}
mysqli_stmt_close($stmt);
?>

<form class="form-horizontal" role="form" id="formulario_nuevo" onsubmit="event.preventDefault(); nuevo('genero')">

<div class="modal-header">
   <h4 class="modal-title">Géneros</h4>
</div>
<br>
      ...
      <table class="table table-striped table-hover">
       ...
       <tbody>
        ...
        <?php if (empty($generos)): ?>
          <tr><td colspan="2">No hay géneros cargados.</td></tr>
        <?php endif; ?>
       </tbody>
      </table>
     </div>
...
$(function() {
  $('.ver_modal').click(function() {
    ...
    $('#panel_inicio').load("clases/modifica/upd-genero.php", {id: id});
  });
});
</script>
